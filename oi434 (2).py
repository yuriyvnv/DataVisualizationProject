...
def update_charts(country, date, audio_feat, genres):
    ...
    fig6 = ff.create_distplot(hist_data, group_labels, bin_size=.2, histnorm='probability density', curve_type='kde',
                              show_hist=False,
                              show_rug=False)
    fig6.update_layout(margin=dict(l=0, r=0, t=40, b=0), modebar_activecolor='#3aa405', showlegend=True,title=f'{audio_feat}'.capitalize() + ' Distplot')
    fig6.update_layout({
        'plot_bgcolor': '#424140',
        'paper_bgcolor': '#424140', },
